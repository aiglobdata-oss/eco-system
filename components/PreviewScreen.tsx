...
function CardSheen({
  ...
  const auraOpacity = breathe.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [0.04, 0.12, 0.05],
  });

  const borderOpacity = breathe.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [0.18, 0.45, 0.22],
  });
  ...
  return (
    ...
    >
      <Animated.View style={[styles.cardIridescentAura, { opacity: auraOpacity }]} />
      <Animated.View
        style={[
          styles.cardIridescentBorder,
          {
            borderRadius: radius,
